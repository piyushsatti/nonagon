...
from nonagon_bot.core.infra.postgres.mappers import character_from_orm, character_to_orm
from nonagon_bot.core.infra.postgres.models import CharacterModel
...
class CharactersRepoPostgres:
    ...
    async def list_by_owner(
        self, guild_id: int, owner_id: UserID
    ) -> List[Character]:
        """Get all characters owned by a user in a guild."""
        async with get_session() as session:
            stmt = select(CharacterModel).where(
                and_(
                    CharacterModel.guild_id == int(guild_id),
                    CharacterModel.owner_id == str(owner_id),
                )
            )
